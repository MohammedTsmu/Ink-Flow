  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <p className="text-sm text-gray-400 mb-4">Select printers from your system to add to Ink Flow.</p>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-10">
            ...
            <span className="ml-3 text-gray-400">Scanning system printers...</span>
          </div>
        ) : systemPrinters.length === 0 ? (
          <p className="text-center text-gray-500 py-8">No printers detected on this system.</p>
        ) : (
          <>
              ...
              {systemPrinters.map((sp) => (
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                    selected.has(sp.name) ? 'bg-blue-600/20 border border-blue-600/50' : 'bg-gray-800 border border-gray-700 hover:border-gray-600'
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">{sp.driverName}</p>
                  </div>
                </label>
              ))}
            </div>
